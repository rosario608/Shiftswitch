...
| A programme with **nothing configured**: naming a shift, posting it, a colleague taking it | `tests/integration/marketplace-first.test.ts` — the programme is created with no services and asserted empty before anything runs |
| Creating and posting in one transaction, and leaving nothing behind when the post is refused | `tests/integration/marketplace-first.test.ts` |
| A model's extraction never writes; the review gate cannot be bypassed by the caller | `tests/integration/marketplace-first.test.ts` |
| Reading a merged-cell week, a month-to-a-page PDF, a people-by-days grid and a photograph | `tests/unit/assisted-import.test.ts`, against recorded responses and the four fixture files |
| An extraction that is unreadable, malformed, over budget, over size, or from another programme | `tests/unit/assisted-import.test.ts`, `tests/integration/marketplace-first.test.ts` |
| Both paths composing: residents' own shifts first, the block file a fortnight later | `tests/integration/marketplace-first.test.ts`, last case |
...
## The assisted import never makes a live call

Every test of it substitutes a `ReplayTransport` that returns a recorded
response, so `npm run verify` stays deterministic and needs no network and no
API key. The recorded responses are **authored rather than captured** — there is
no key in the environment this repository is developed in — and
`tests/fixtures/assisted-import/responses.ts` says so at the top rather than
implying otherwise.

What that buys and what it does not: these tests prove our handling is right —
the flagging, the ordering, the gate, the commit path, the honest failures — and
they do not prove that a model reads a merged cell correctly. Nothing offline
can prove the second. The transport is a seam so that a real capture can replace
the constants without anything else changing.

The four input files are generated by `npx tsx scripts/make-import-fixtures.ts`
and committed. They are built in code rather than checked in as opaque binaries
so that what each one *is* — where the merge spans, which column is the legend —
is readable in the script.
